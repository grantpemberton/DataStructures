...
void read_lyrics(const char * filename, bool show_progress)
{
	...
	while ( ! in.eof() ) 
	{
		...
		getline(in, artist);
		if (in.fail()) break;
		//insert this into the SongVector
		...
		while ( in >> word && word != "<BREAK>" ){
			//
			//insert these words into the hashtable, the counter 
			//will keep track of which song they are coming from 
			//in the large song vector
