...
public:
	...
	void insert_at_head(string c); // inserts at the beginning of the list
	void insert_at_tail(string c); // inserts at the end of the list
	void insert_at_index(string c, int index); // inserts at an index
	void replace_at_index(string c, int index); // replaces the card
						// at an index
	Card card_at(int index); // returns the card at the index
	
	bool has_card(string c); // returns true if the card is in the list
	
	bool remove(string c); // removes the card
	...
	int lyrics_in_array() { return lyrics_held; }
	
private:
	void expand(); // expand the list when necessary
	string *lyrics; // the array of cards
	int lyrics_held; // how many cards are currently in the hand
	int lyric_capacity; // the capacity of the array that holds the cards
};
