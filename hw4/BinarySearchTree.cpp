...
//insert wtih fewer args, public insert function
...
//returns the height of the tree who's root is node
	...
	if(node == NULL) return -1;//Base Case
		...
		leftHeight = tree_height(node->left);//recursive traversal left
		rightHeight = tree_height(node->right);//recursive traversal right
	}
	if (leftHeight > rightHeight){
		return leftHeight+1;//add 1 at the end to account for root
	}else{
		return rightHeight +1;//add 1 at the end to account for root
	}
}
